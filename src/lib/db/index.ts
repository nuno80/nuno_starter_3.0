...
if (!fs.existsSync(dbDir)) {
  console.log(
    `[DB_INIT] Database directory ${dbDir} does not exist. Attempting to create...`
  );
  try {
...
try {
  console.log(
    `[DB_INIT] Attempting to connect to/create SQLite database at: ${dbPath}`
  );
  ...
  console.log(
    `[DB_INIT] Successfully connected to/created SQLite database: ${dbPath}`
  );
  console.log(
    `[DB_INIT] Database file exists on disk: ${fs.existsSync(dbPath)}`
  );
